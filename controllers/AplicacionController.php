...
class AplicacionController extends ActiveRecord
{
    ...
    public static function guardarAPI()
    {
        ...
        $nombreExistente = self::fetchFirst("SELECT app_id FROM jemg_aplicaciones WHERE app_nombre_largo = '{$_POST['app_nombre_largo']}'");
        if ($nombreExistente) {
            http_response_code(400);
            echo json_encode([
                'codigo' => 0,
        ...
        $nombreCortoExistente = self::fetchFirst("SELECT app_id FROM jemg_aplicaciones WHERE app_nombre_corto = '{$_POST['app_nombre_corto']}'");
    ...
    public static function buscarAplicaciones()
    {
        ...
        try {
            $sql = "SELECT * FROM jemg_aplicaciones WHERE app_situacion = 1";
    ...
    public static function modificarAplicacion()
    {
        ...
        $nombreExistente = self::fetchFirst("SELECT app_id FROM jemg_aplicaciones WHERE app_nombre_largo = '{$_POST['app_nombre_largo']}' AND app_id != $id");
        if ($nombreExistente) {
            http_response_code(400);
            echo json_encode([
                'codigo' => 0,
        ...
        $nombreCortoExistente = self::fetchFirst("SELECT app_id FROM jemg_aplicaciones WHERE app_nombre_corto = '{$_POST['app_nombre_corto']}' AND app_id != $id");
        if ($nombreCortoExistente) {
            http_response_code(400);
            echo json_encode([
                'codigo' => 0,
    ...
    public static function eliminarAplicacion()
    {
        ...
        try {
            ...
            $consulta = "UPDATE jemg_aplicaciones SET app_situacion = 0 WHERE app_id = $id";
            self::SQL($consulta);
